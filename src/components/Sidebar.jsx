      ...
      <aside className={`sv-sidebar ${collapsed ? 'is-collapsed' : ''} ${mobileOpen ? 'is-mobile-open' : ''}`}>
        ...
        <nav className="sv-sidebar-nav" aria-label="Main navigation">
          {NAV_ITEMS.map(([to, icon, label]) => (
            <NavLink key={to} to={to} end={to === '/'} onClick={closeMobile} className={({ isActive }) => `sv-sidebar-link ${isActive ? 'active' : ''}`}>
              <span className="sv-sidebar-icon">{icon}</span>
        ...
        <div className="sv-sidebar-bottom">
          {SECONDARY_ITEMS.map(([to, icon, label]) => (
            <NavLink key={to} to={to} onClick={closeMobile} className={({ isActive }) => `sv-sidebar-link ${isActive ? 'active' : ''}`}>
              <span className="sv-sidebar-icon">{icon}</span>
              {!collapsed && <span>{label}</span>}
            </NavLink>
          ))}

          <button className="sv-sidebar-collapse" onClick={() => setCollapsed(value => !value)}>
            <span>{collapsed ? '→' : '←'}</span>
