...
export default function ProfileMenu() {
  ...
  return <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
    <div ref={notificationRef} style={{ position: 'relative' }}>
      <button type="button" onClick={() => setNotificationsOpen(v => !v)} aria-label="Notifications" style={{ position:'relative', width:42, height:42, borderRadius:12, border:'1px solid var(--app-border)', background:'var(--app-surface)', color:'var(--app-text)', cursor:'pointer', fontSize:19 }}>
      ...
      {notificationsOpen && <div style={{ position:'absolute', right:0, top:50, width:'min(390px, calc(100vw - 28px))', maxHeight:520, overflow:'hidden', zIndex:120, background:'var(--app-surface)', border:'1px solid var(--app-border)', borderRadius:16, boxShadow:'var(--app-shadow)' }}>
        <div style={{ display:'flex', justifyContent:'space-between', padding:'15px 16px', borderBottom:'1px solid var(--app-border)' }}><div><strong>Notifications</strong><div style={{fontSize:11,color:'var(--app-muted)',marginTop:3}}>{unreadCount ? `${unreadCount} unread` : 'All caught up'}</div></div>{unreadCount > 0 && <button onClick={markAllRead} style={{border:0,background:'transparent',color:'var(--app-accent)',cursor:'pointer',fontSize:12}}>Mark all read</button>}</div>
