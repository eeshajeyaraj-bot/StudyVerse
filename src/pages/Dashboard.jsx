...
export default function Dashboard() {
  ...
  useEffect(() => { Promise.all([fetchSessions(), fetchPlayerStats()]).finally(() => setLoading(false)) }, [])
  ...
  async function fetchPlayerStats() { const { data:{user} }=await supabase.auth.getUser(); if(!user)return; const {data}=await supabase.from('player_stats').select('*').eq('user_id',user.id); if(data?.length)setPlayerStats(data[0]) }
  ...
  const xp=playerStats?playerStats.xp%100:0
  ...
  return <div className="sv-page"><div className="sv-container sv-dashboard-page">
    <div className="sv-page-header"><div><p className="sv-eyebrow">Overview</p><h1>Good to see you back.</h1><p className="sv-eyebrow" style={{marginTop:8}}>Your study activity, goals and recent sessions in one place.</p></div>{playerStats&&<div className="sv-level-card"><div><strong>Level {playerStats.level}</strong><span>{xp}/100 progress</span></div><div className="sv-progress-bg"><div className="sv-progress-fill" style={{width:`${xp}%`}}/></div></div>}</div>
