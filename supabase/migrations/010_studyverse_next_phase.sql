...
-- Membership management: a member can manage only their own membership;
-- the room host may remove members and add members explicitly.
...
for insert to authenticated with check (
  user_id = auth.uid()
  or exists (select 1 from public.study_rooms r where r.id = room_members.room_id and r.host_id = auth.uid())
);
...
for update to authenticated using (
  user_id = auth.uid()
  or exists (select 1 from public.study_rooms r where r.id = room_members.room_id and r.host_id = auth.uid())
) with check (user_id = auth.uid() or exists (select 1 from public.study_rooms r where r.id = room_members.room_id and r.host_id = auth.uid()));
...
-- Lightweight database-side anti-spam protection: max 30 messages/minute/user.
create or replace function public.studyverse_rate_limit_message()
returns trigger language plpgsql security definer set search_path = public as $$
declare recent_count integer;
begin
  select count(*) into recent_count from public.direct_messages
...
create or replace function public.studyverse_rate_limit_room_message()
returns trigger language plpgsql security definer set search_path = public as $$
declare recent_count integer;
begin
  select count(*) into recent_count from public.room_messages
  where sender_id = auth.uid() and created_at > now() - interval '1 minute';
  if recent_count >= 30 then raise exception 'Message rate limit reached. Please wait a moment.'; end if;
  return new;
end;
